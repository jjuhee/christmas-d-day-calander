...
doors.forEach((door, index) => {
  door.addEventListener("click", () => {
    ...
    if (now.getTime() > openDate.getTime()) {
      ...
      const prevDoorInput = document.querySelector(
        `.day-${index} > label > input`
      );
      const doorInput = document.querySelector(
        `.day-${index + 1} > label > input`
      );
      ...
      if (index === 0 || prevDoorInput.checked) {
        ...
        showModal("image/card/jjoo1.jpg", "이전카드를 먼저 열어줘..", index);
      }
    } else {
